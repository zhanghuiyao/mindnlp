...
STOPPING_CRITERIA_INPUTS_DOCSTRING = r"""
    ...
    Return:
        `ms.Tensor`. (`ms.Tensor` of shape `(batch_size, 1)`), where `True` indicates we stop generation
...
class StopStringCriteria(StoppingCriteria):
    ...
    >>> inputs = tokenizer("The biggest states in the USA by land area:", return_tensors="np")
    ...
    def __init__(self, tokenizer: PreTrainedTokenizerBase, stop_strings: Union[str, List[str]]):
        ...
        self.target_lens = ms.Tensor([len(stop_string) for stop_string in stop_strings], dtype=ms.int32)
    ...
    @staticmethod
    def _stop_string_create_embedding_vec(token_list, token_indices, stop_strings) -> Dict[str, ms.Tensor]:
        ...
        for i, stop_string in enumerate(stop_strings):
            ...
            # than mindspore for speed + simplicity, then convert to mindspore at the end
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor, **kwargs) -> ms.Tensor:
        ...
        flipped_ids = ops.flip(input_ids, (1,))
        ...
        embedded = ops.embedding(flipped_ids, self.embedding_vec)
        ...
        # valid_positions = valid_positions.unflatten(-1, (self.num_stop_strings, -1))  # FIXME: not support on MindSpore 2.3.1
        valid_positions = valid_positions.view(valid_positions.shape[:-1] + (self.num_stop_strings, -1))
        ...
        # end_lengths = end_lengths.unflatten(-1, (self.num_stop_strings, -1))   # FIXME: not support on MindSpore 2.3.1
        end_lengths = end_lengths.view(end_lengths.shape[:-1] + (self.num_stop_strings, -1))
        ...
        lengths = lengths.broadcast_to((-1, -1, end_lengths.shape[-2], end_lengths.shape[-1]))
        lengths_with_ends = ops.cat([end_lengths, lengths], axis=1)

        # cumsum() to get the number of matched characters in the stop string after each token
        cumsum = lengths_with_ends.cumsum(axis=1)  # B x maximum_token_len x num_stop_strings x max_valid_end_lens
        ...
        later_match = ops.any(cumsum[:, :-1, :, None] == valid_positions[:, :, :, :, None], axis=-2)
        ...
        string_matches = ops.amax(cumsum * mask, axis=(1, -1)) >= self.target_lens[None, :]

        # We return a per-sample vector that is True if any stop string is matched for that sample
        return ops.any(string_matches, axis=-1)
