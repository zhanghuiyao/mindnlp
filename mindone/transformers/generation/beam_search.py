...
PROCESS_INPUTS_DOCSTRING = r"""
    Args:
        ...
        next_scores (`ms.Tensor` of shape `(batch_size, 2 * num_beams)`):
            Current scores of the top `2 * num_beams` non-finished beam hypotheses.
        next_tokens (`ms.Tensor` of shape `(batch_size, 2 * num_beams)`):
            `input_ids` of the tokens corresponding to the top `2 * num_beams` non-finished beam hypotheses.
        next_indices (`ms.Tensor` of shape `(batch_size, 2 * num_beams)`):
    ...
    Return:
        `UserDict`: A dictionary composed of the fields as defined above:
            ...
            - **next_beam_scores** (`ms.Tensor` of shape `(batch_size * num_beams)`) -- Updated scores of all
              non-finished beams.
            - **next_beam_tokens** (`ms.Tensor` of shape `(batch_size * num_beams)`) -- Next tokens to be added
              to the non-finished beam_hypotheses.
            - **next_beam_indices** (`ms.Tensor` of shape `(batch_size * num_beams)`) -- Beam indices
...
FINALIZE_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`ms.Tensor` of shape `(batch_size * num_beams, sequence_length)`):
            ...
            [What are input IDs?](../glossary#input-ids)
        final_beam_scores (`ms.Tensor` of shape `(batch_size * num_beams)`):
            The final scores of all non-finished beams.
        final_beam_tokens (`ms.Tensor` of shape `(batch_size * num_beams)`):
            The last tokens to be added to the non-finished beam_hypotheses.
        final_beam_indices (`ms.Tensor` of shape `(batch_size * num_beams)`):
    ...
    Return:
        `ms.Tensor` of shape `(batch_size * num_return_sequences, sequence_length)`: The generated sequences.
...
class BeamSearchScorer(BeamScorer):
    ...
    def finalize(
        ...
        sent_lengths = input_ids.new_zeros(batch_size * self.num_beam_hyps_to_keep)
        best = []
        best_indices = []
        ...
        decoded: ms.Tensor = input_ids.new_zeros((batch_size * self.num_beam_hyps_to_keep, sent_max_len))

        if len(best_indices) > 0 and best_indices[0] is not None:
            indices: ms.Tensor = input_ids.new_zeros((batch_size * self.num_beam_hyps_to_keep, sent_max_len))
        else:
            indices = None
...
class ConstrainedBeamSearchScorer(BeamScorer):
    ...
    def __init__(
        ...
        self._done = ms.Tensor([False for _ in range(batch_size)], dtype=ms.bool_)
    ...
    def process(
        ...
        Args:
            ...
            next_indices (`ms.Tensor` of shape `(batch_size, 2 * num_beams)`):
                Beam indices indicating to which beam hypothesis the `next_tokens` correspond.
            scores_for_all_vocab (`ms.Tensor` of shape `(batch_size * num_beams, sequence_length)`):
        ...
        Return:
            `UserDict`: A dictionary composed of the fields as defined above:
                ...
                - **next_beam_scores** (`ms.Tensor` of shape `(batch_size * num_beams)`) -- Updated scores of
                  all
                non-finished beams.

                - **next_beam_tokens** (`ms.Tensor` of shape `(batch_size * num_beams)`) -- Next tokens to be
                  added
                to the non-finished beam_hypotheses.
                - **next_beam_indices** (`ms.Tensor` of shape `(batch_size * num_beams)`) -- Beam indices
                indicating to which beam the next tokens shall be added.
    ...
    def step_sentence_constraint(
        ...
        batch_idx: int,
        input_ids: ms.Tensor,
        vocab_scores: ms.Tensor,
        sent_beam_scores: ms.Tensor,
        sent_beam_tokens: ms.Tensor,
        sent_beam_indices: ms.Tensor,
        ...
        for seq_idx, pre_seq in enumerate(this_batch_input_ids):
            ...
            if not advance_state.completed:
                advance_tokens = ms.Tensor(advance_state.advance())
        ...
        if len(track_new["new_indices"]) > 0:
            new_indices = ms.Tensor(track_new["new_indices"])
            ...
            all_banks = ms.Tensor([one.get_bank() for one in all_states])
            ...
            rearrangers = ms.Tensor(np.argsort(increments, kind="mergesort"))
