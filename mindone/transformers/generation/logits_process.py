...
from ..mindspore_adapter.utils import dtype_to_min, dtype_to_max

NEG_FLOAT_INF = dtype_to_min(ms.float16)  # In order to adapt to more general scenarios without overflow, float16 type is used here.
FLOAT_INF = -NEG_FLOAT_INF
...
class MinLengthLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, min_length: int, eos_token_id: Union[int, List[int], ms.Tensor]):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        if input_ids.shape[-1] < self.min_length:
            scores_processed = ops.where(eos_token_mask, NEG_FLOAT_INF, scores)
...
class RepetitionPenaltyLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        scores_processed = ops.tensor_scatter_elements(scores, input_ids, score, axis=1)
        return scores_processed
...
class EncoderRepetitionPenaltyLogitsProcessor(LogitsProcessor):
    ...
    >>> inputs = tokenizer(["Alice and Bob. The third member's name was"], return_tensors="np")
    >>> inputs = ms.Tensor(inputs)
    >>> gen_out = model.generate(**inputs)
    >>> print(tokenizer.batch_decode(gen_out, skip_special_tokens=True)[0])
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        scores_processed = ops.tensor_scatter_elements(scores, self.encoder_input_ids, score, axis=1)
        return scores_processed
...
class TopPLogitsWarper(LogitsProcessor):
    ...
    def __init__(self, top_p: float, filter_value: float = NEG_FLOAT_INF, min_tokens_to_keep: int = 1):
        top_p = float(top_p)
...
class TopKLogitsWarper(LogitsProcessor):
    ...
    def __init__(self, top_k: int, filter_value: float = NEG_FLOAT_INF, min_tokens_to_keep: int = 1):
...
class MinPLogitsWarper(LogitsProcessor):
    ...
    def __init__(self, min_p: float, filter_value: float = NEG_FLOAT_INF, min_tokens_to_keep: int = 1):
        if not (0 <= min_p <= 1.0):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        top_probs, _ = probs.max(axis=-1, keepdims=True, return_indices=True)
        # Calculate the actual min_p threshold by scaling min_p with the top token's probability
        scaled_min_p = self.min_p * top_probs
        # Create a mask for tokens that have a probability less than the scaled min_p
        tokens_to_remove = probs < scaled_min_p

        sorted_indices = ops.argsort(scores, axis=-1, descending=True)
        sorted_indices_to_remove = ops.gather_elements(tokens_to_remove, dim=-1, index=sorted_indices)
        # Keep at least min_tokens_to_keep
...
class TypicalLogitsWarper(LogitsProcessor):
    ...
    def __init__(self, mass: float = 0.9, filter_value: float = NEG_FLOAT_INF, min_tokens_to_keep: int = 1):
        mass = float(mass)
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        ent = -(normalized * p).nansum(-1, keepdims=True)

        # shift and sort
        shifted_scores = ops.abs((-normalized) - ent)
        sorted_scores, sorted_indices = ops.sort(shifted_scores, descending=False)
        sorted_logits = ops.gather_elements(scores, -1, sorted_indices)
        cumulative_probs = sorted_logits.softmax(axis=-1).cumsum(axis=-1)

        # Remove tokens with cumulative mass above the threshold
        last_ind = (cumulative_probs < self.mass).sum(axis=1)

        last_ind.clamp(max=sorted_scores.shape[-1] - 1)
        sorted_indices_to_remove = sorted_scores > ops.gather_elements(sorted_scores, 1, last_ind.view(-1, 1))
        sorted_indices_to_remove[..., : self.min_tokens_to_keep] = 0
        indices_to_remove = ops.tensor_scatter_elements(sorted_indices_to_remove, sorted_indices, sorted_indices_to_remove, axis=1)
...
class EpsilonLogitsWarper(LogitsProcessor):
    ...
    def __init__(self, epsilon: float, filter_value: float = NEG_FLOAT_INF, min_tokens_to_keep: int = 1):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        indices_to_remove = ops.logical_and(indices_to_remove, scores < ops.topk(scores, top_k)[0][..., -1, None])
...
class EtaLogitsWarper(LogitsProcessor):
    ...
    def __init__(
        self, epsilon: float, filter_value: float = NEG_FLOAT_INF, min_tokens_to_keep: int = 1
    ):
        epsilon = float(epsilon)
        if epsilon <= 0 or epsilon >= 1:
        ...
        self.epsilon = ms.Tensor(epsilon)
        self.filter_value = filter_value
        self.min_tokens_to_keep = min_tokens_to_keep
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        # get entropy with Categorical distribution
        logits = scores - scores.logsumexp(axis=-1, keepdims=True)   # get logits
        probs = ops.softmax(logits, axis=-1)
        min_real = dtype_to_min(logits.dtype)
        logits = ops.clamp(logits, min=min_real)
        p_log_p = logits * probs
        entropy = -p_log_p.sum(-1)

        eta = ops.minimum(self.epsilon, ops.sqrt(self.epsilon) * ops.exp(-entropy))[..., None]
...
class NoRepeatNGramLogitsProcessor(LogitsProcessor):
    ...
    >>> model = AutoModelForCausalLM.from_pretrained("distilbert/distilgpt2")
    >>> tokenizer = AutoTokenizer.from_pretrained("distilbert/distilgpt2")
    >>> inputs = tokenizer(["Today I"], return_tensors="np")
    >>> inputs = ms.Tensor(inputs)
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        for i, banned_tokens in enumerate(banned_batch_tokens):
            scores_processed[i, banned_tokens] = NEG_FLOAT_INF
...
class SequenceBiasLogitsProcessor(LogitsProcessor):
    ...
    >>> model = AutoModelForCausalLM.from_pretrained("openai-community/gpt2")
    >>> tokenizer = AutoTokenizer.from_pretrained("openai-community/gpt2")
    >>> inputs = tokenizer(["The full name of Donald is Donald"], return_tensors="np")
    >>> inputs = ms.Tensor(inputs)
    ...
    def _prepare_bias_variables(self, scores: ms.Tensor):
        ...
        # with simpler logic.
        self.length_1_bias = ops.zeros((vocabulary_size,), dtype=ms.float32)
...
class NoBadWordsLogitsProcessor(SequenceBiasLogitsProcessor):
    ...
    >>> model = AutoModelForCausalLM.from_pretrained("openai-community/gpt2")
    >>> tokenizer = AutoTokenizer.from_pretrained("openai-community/gpt2")
    >>> inputs = tokenizer(["In a word, the cake is a"], return_tensors="np")
    >>> inputs = ms.Tensor(inputs)
    ...
    def __init__(
        self, bad_words_ids: List[List[int]], eos_token_id: Optional[Union[int, List[int], ms.Tensor]] = None
    ):
        ...
        if eos_token_id is not None:
            ...
            bad_words_ids = list(
                filter(lambda bad_token_seq: all(bad_token_seq != [int(i)] for i in eos_token_id), bad_words_ids)
            )

        # Forbidding a sequence is equivalent to setting its bias to -inf
        sequence_bias = {tuple(sequence): NEG_FLOAT_INF for sequence in bad_words_ids}
...
class PrefixConstrainedLogitsProcessor(LogitsProcessor):
    ...
    >>> entity = tokenizer(" Bob Marley", return_tensors="np").input_ids[0]  # 3 tokens
    >>> entity = ms.Tensor(entity)
    ...
    @add_start_docstrings(LOGITS_PROCESSOR_INPUTS_DOCSTRING)
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        mask = ops.full_like(scores, NEG_FLOAT_INF)
...
class ForcedBOSTokenLogitsProcessor(LogitsProcessor):
    ...
    >>> inputs = tokenizer("Translate from English to German: I love cats.", return_tensors="np")
    >>> inputs = ms.Tensor(inputs)
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        if cur_len == 1:
            scores_processed = ops.full_like(scores, NEG_FLOAT_INF)
...
class InfNanRemoveLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        # scores_processed = ops.where(scores != scores, 0.0, scores)   # can't work on MindSpore 2.3.1
        scores_processed = ops.nan_to_num(scores, nan=0.0)

        # set all +/-inf values to max/min possible value
        scores_processed = ops.where(scores == FLOAT_INF, dtype_to_max(scores.dtype), scores_processed)
        scores_processed = ops.where(scores == NEG_FLOAT_INF, dtype_to_min(scores.dtype), scores_processed)
...
class ExponentialDecayLengthPenalty(LogitsProcessor):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        penalties = ops.zeros_like(scores)
        scores_processed = scores
        if cur_len > self.regulation_start:
            ...
            penalty = ops.abs(scores[:, self.eos_token_id]) * (pow(self.regulation_factor, penalty_idx) - 1)
...
class LogitNormalization(LogitsProcessor):
    ...
    >>> print(np.allclose(ops.sum(ops.exp(outputs.scores[-1])).asnumpy(), np.array((1.000,)), rtol=1e-4))
    False
...
class SuppressTokensAtBeginLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        if input_ids.shape[-1] == self.begin_index:
            scores_processed = ops.where(suppress_token_mask, NEG_FLOAT_INF, scores)
...
class SuppressTokensLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        scores = ops.where(suppress_token_mask, NEG_FLOAT_INF, scores)
        return scores
...
class WhisperTimeStampLogitsProcessor(LogitsProcessor):
    ...
    >>> model = WhisperForConditionalGeneration.from_pretrained("openai/whisper-tiny.en")
    >>> ds = load_dataset("hf-internal-testing/librispeech_asr_dummy", "clean", split="validation")
    >>> inputs = processor(ds[3]["audio"]["array"], return_tensors="np")
    >>> input_features = ms.Tensor(inputs.input_features)
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        scores_processed = scores.clone()
        scores_processed[:, self.no_timestamps_token_id] = NEG_FLOAT_INF
        ...
        for k in range(input_ids.shape[0]):
            ...
            if last_was_timestamp:
                if penultimate_was_timestamp:  # has to be non-timestamp
                    scores_processed[k, self.timestamp_begin :] = NEG_FLOAT_INF
                else:  # cannot be normal text tokens
                    scores_processed[k, : self.eos_token_id] = NEG_FLOAT_INF
            ...
            if timestamps.numel() > 0:
                ...
                scores_processed[k, self.timestamp_begin : timestamp_last] = NEG_FLOAT_INF

        # apply the `max_initial_timestamp` option
        if input_ids.shape[1] == self.begin_index:
            scores_processed[:, : self.timestamp_begin] = NEG_FLOAT_INF

            if self.max_initial_timestamp_index is not None:
                last_allowed = self.timestamp_begin + self.max_initial_timestamp_index
                scores_processed[:, last_allowed + 1 :] = NEG_FLOAT_INF

        # if sum of probability over timestamps is above any other token, sample timestamp
        logprobs = ops.log_softmax(scores_processed.float(), axis=-1)
        for k in range(input_ids.shape[0]):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        if input_ids.shape[1] == self.begin_index:
            ...
            if is_scores_logprobs:
                ...
                probs = no_speech_scores.float().softmax(axis=-1)
...
class AlternatingCodebooksLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        if is_first_codebook:
            scores_processed[:, : self.semantic_vocab_size] = NEG_FLOAT_INF
            scores_processed[:, self.semantic_vocab_size + self.codebook_size :] = NEG_FLOAT_INF
        else:
            scores_processed[:, : self.semantic_vocab_size + self.codebook_size] = NEG_FLOAT_INF
...
class UnbatchedClassifierFreeGuidanceLogitsProcessor(LogitsProcessor):
    ...
    >>> neg_inputs = tokenizer(["A very happy event happened,"], return_tensors="np")
    >>> neg_inputs = ms.Tensor(neg_inputs)
...
class BarkEosPrioritizerLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        if self.min_eos_p:
            ...
            early_stop_scores = ops.ones_like(scores) * NEG_FLOAT_INF
            ...
            scores_processed = ops.where(do_early_stop, early_stop_scores, scores)
...
class WatermarkLogitsProcessor(LogitsProcessor):
    ...
    >>> model = AutoModelForCausalLM.from_pretrained("openai-community/gpt2")
    >>> tokenizer = AutoTokenizer.from_pretrained("openai-community/gpt2")
    >>> inputs = tokenizer(["Alice and Bob are"], return_tensors="np")
    >>> inputs = ms.Tensor(inputs)
    ...
    def __init__(
        ...
        self.rng = hashing_key
        self.hash_key = hashing_key
        self.context_width = context_width

        self.table_size = 1_000_003
        self.fixed_table = ops.randperm(self.table_size, seed=self.rng)
    ...
    def set_seed(self, input_seq: ms.Tensor):
        ...
        self.rng = seed % (2**64 - 1)

    def _get_greenlist_ids(self, input_seq: ms.Tensor) -> ms.Tensor:
        self.set_seed(input_seq)
        vocab_permutation = ops.randperm(self.vocab_size, seed=self.rng)
    ...
    def _score_rejection_sampling(self, input_seq: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        """
        final_greenlist = []
        _, greedy_predictions = scores.sort(axis=-1, descending=True)

        # 40 is an arbitrary number chosen to save compute and not run for long (taken from orig repo)
        for i in range(40):
            greenlist_ids = self._get_greenlist_ids(ops.cat([input_seq, greedy_predictions[i, None]], axis=-1))
...
class SynthIDTextWatermarkState:
    ...
    def __init__(
        ...
        """
        self.context = ops.zeros(
            (batch_size, ngram_len - 1),
            dtype=ms.int64,
        )
        self.context_history = ops.zeros(
...
class SynthIDTextWatermarkLogitsProcessor(LogitsProcessor):
    ...
    * **Utility Functions:** Provides functions to compute g-values directly, check for context repetition, create
    ...
    def __init__(
        ...
        self.sampling_table = ops.randint(
            low=0,
            high=2,
            size=(sampling_table_size,),
            seed=sampling_table_seed,
        )
    ...
    def update_scores(self, scores: ms.Tensor, g_values: ms.Tensor) -> ms.Tensor:
        ...
        log_probs = ops.log(probs)
        log_probs = ops.where(ops.isfinite(log_probs), log_probs, dtype_to_min(log_probs.dtype))
        return log_probs
    ...
    def __call__(self, input_ids: ms.Tensor, scores: ms.Tensor) -> ms.Tensor:
        ...
        if self.debug_mode:
            scores = ops.ones_like(scores)

        # Currently indices is just a arange to compute watermarking on the desnse logits.
        all_indices = ops.stack([ops.arange(vocab_size) for _ in range(batch_size)])
        ...
        self.state.context_history = ops.concat(
            (hash_result_with_just_context, self.state.context_history),
            axis=1,
        )[:, :-1]

        updated_watermarked_scores = ops.where(
            is_repeated_context,
            input=scores,
    ...
    def accumulate_hash(
        ...
        for i in range(data.shape[-1]):
            current_hash = ops.add(current_hash, data[..., i])
            current_hash = ops.mul(current_hash, multiplier)
            current_hash = ops.add(current_hash, increment)
        return current_hash
    ...
    def compute_ngram_keys(self, ngrams: ms.Tensor) -> ms.Tensor:
        ...
        hash_result = ops.ones(batch_size, dtype=ms.int32)
    ...
    def _compute_keys(
        self, n_minus_1_grams: ms.Tensor, indices: ms.Tensor
    ...
    def sample_g_values(self, ngram_keys: ms.Tensor) -> ms.Tensor:
        ...
        # not support on MindSpore 2.3.1
        # return ops.take_along_dim(sampling_table, indices=ngram_keys, dim=2)

        output_shape = ngram_keys.shape[:-1] + (sampling_table_size,)
        return ops.gather_elements(
            ops.broadcast_to(sampling_table, output_shape),
            dim=2,
            index=ngram_keys
        )
    ...
    def compute_g_values(self, input_ids: ms.Tensor) -> ms.Tensor:
        ...
        # unfold
        # ngrams = input_ids.unfold(dimension=1, size=self.ngram_len, step=1), not support on MindSpore 2.3.1
        dimension, size, step = 1, self.ngram_len, 1
        b, n = input_ids.shape[0], input_ids.shape[1] - size + 1  # when step=1
        gather_indexes = (ops.arange(n)[:, None] + ops.arange(size)[None, :]).reshape(-1)
        ngrams = input_ids[:, gather_indexes].reshape(b, n, -1)
    ...
    def compute_context_repetition_mask(self, input_ids: ms.Tensor) -> ms.Tensor:
        ...
        # unfold
        # contexts = input_ids[:, :-1].unfold(dimension=1, size=self.ngram_len - 1, step=1), not support on MindSpore 2.3.1
        dimension, size, step = 1, self.ngram_len - 1, 1
        b, n = input_ids.shape[0], input_ids.shape[1] - size + 1  # when step=1
        gather_indexes = (ops.arange(n)[:, None] + ops.arange(size)[None, :]).reshape(-1)
        contexts = input_ids[:, :-1][:, gather_indexes].reshape(b, n, -1)
        ...
        for i in range(num_contexts):
            ...
            state.context_history = ops.concat(
                (context_hash, state.context_history),
                axis=1,
            )[:, :-1]
        are_repeated_contexts = ops.concat(are_repeated_contexts, axis=1)

        return ops.logical_not(are_repeated_contexts)
    ...
    def compute_eos_token_mask(self, input_ids: ms.Tensor, eos_token_id: int) -> ms.Tensor:
        ...
        for eos_equated in all_eos_equated:
            nonzero_idx = ops.nonzero(eos_equated)
            noneos_mask = ops.ones_like(eos_equated)
            if nonzero_idx.shape[0] != 0:
                noneos_mask[nonzero_idx[0][0] :] = 0
            noneos_masks.append(noneos_mask)
        return ops.stack(noneos_masks, axis=0)
